...
class ModelTrainer():
    def __init__(self, up_model: nn.Module, down_layer: nn.Module = None, train_dataset=None,
        ...
        if isinstance(train_dataset, data.IterableDataset):
            self._train_dataloader = DataLoader(train_dataset, batch_size=None, num_workers=8) #todo: add this to argparser
            ...
            self._train_dataloader = DataLoader(train_dataset, shuffle=data_loader_shuffle,
                                                batch_size=self._train_batch_size, num_workers=8)
            self._steps_per_epoch = len(self._train_dataloader)

        if isinstance(dev_dataset, data.IterableDataset):
            dev_dataloader = DataLoader(dev_dataset, batch_size=None, num_workers=8) #todo: add this to argparser
        else:
            dev_dataloader = DataLoader(dev_dataset, shuffle=data_loader_shuffle, batch_size=self._dev_batch_size, num_workers=8)
